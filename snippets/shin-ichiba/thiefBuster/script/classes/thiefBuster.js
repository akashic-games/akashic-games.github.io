...
var ThiefBuster = /** @class */function (_super) {
  ...
  ThiefBuster.prototype.init = function () {
    ...
    // 赤数字
    var imgStgNum2 = this.scene.asset.getImageById(assetInfo_1.AssetInfo.numRed.img);
    var jsonStgNum2 = this.scene.asset.getJSONContentById(assetInfo_1.AssetInfo.numRed.json);
    ...
    var fontStgNum2 = new g.BitmapFont({
      src: imgStgNum2,
      map: fontmapStgNum2,
    ...
    // 青数字
    var imgStgNum3 = this.scene.asset.getImageById(assetInfo_1.AssetInfo.numBlue.img);
    var jsonStgNum3 = this.scene.asset.getJSONContentById(assetInfo_1.AssetInfo.numBlue.json);
    ...
    var fontStgNum3 = new g.BitmapFont({
      src: imgStgNum3,
      map: fontmapStgNum3,
    ...
    // レイヤー
    this.layerBuilding = new g.E({
      scene: scene
    });
    this.layerThief = new g.E({
      scene: scene
    });
    this.layerBullet = new g.E({
      scene: scene
    });
    this.layerItem = new g.E({
      scene: scene
    });
    ...
    this.score.createScoreLabel(fontStgNum1, define_1.define.GAME_SCORE_DIGIT, {
      x: define_1.define.GAME_SCORE_X,
      y: define_1.define.GAME_SCORE_Y
    });
  ...
  ThiefBuster.prototype.startGame = function () {
    ...
    this.retTimeIdentifier = this.scene.setInterval(function () {
    ...
    }, this.timerLabel.getTimeCount() * 1000 / len,
    // ゲーム制限時間のフェーズ数分の1ごと
    this // owner
    );
  };
   ...
   * @override
   */
  ThiefBuster.prototype.handleUpdateScene = function () {
    if (this.inGame) {
   ...
   */
  ThiefBuster.prototype.handleTimeCaution = function () {
    this.timeCaution.fire();
  };
  /**
   * TimerLabel#timeCautionCancelのハンドラ
   */
  ThiefBuster.prototype.handleTimeCautionCancel = function () {
   ...
   */
  ThiefBuster.prototype.handleTouch = function (_e) {
    if (!this.inGame) {
      return true;
    }
  ...
  ThiefBuster.prototype.createDoor = function (_index) {
    ...
    doorActor.onUpdate.add(function () {
      doorActor.modified();
      doorActor.calc();
    });
    return doorActor;
  ...
  ThiefBuster.prototype.sortArrayRandom = function (_ary) {
    ...
    while (len) {
      var randWk = Math.floor(this.scene.game.random.generate() * (len + 1));
      var wk = _ary[len];
  ...
  ThiefBuster.prototype.popThiefController = function () {
    ...
    if (this.cntGame === 0 || this.cntGame % info.popInterval === 0) {
      // 段階ごとの泥棒出現フレーム
