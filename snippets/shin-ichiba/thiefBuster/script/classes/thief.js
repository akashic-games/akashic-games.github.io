...
var Thief = /** @class */function () {
  ...
  function Thief(_scene, _parent, _type, _floor) {
    ...
    if (typeof _type === "undefined") {
      // 引数なければ
      thiefType = Math.floor(this.game.random.generate() * define_1.define.NUM_OF_THIEF_TYPE);
    } else {
      thiefType = _type;
    }
   ...
   */
  Thief.prototype.handleUpdate = function () {
    if (!this.spr.destroyed()) {
  ...
  Thief.prototype.setAnime = function (_animetype) {
    ...
    if (this.spr.animation.name === this.animeTypes.in) {
      this.escapeStartTrigger.addOnce(function () {
