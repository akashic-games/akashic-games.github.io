...
var Score = /** @class */function (_super) {
  ...
  Score.prototype.createScoreLabel = function (_font, _digit, _posStart) {
    ...
    entityUtil_1.entityUtil.moveNumLabelTo(
    // 1ケタ目左上へ移動
    this.label, _posStart.x, _posStart.y, _font);
    // ラベル初期位置記憶
    this.posLabelStart = {
      x: this.label.x,
      y: this.label.y
   ...
   * ラベルテキストの更新
   */
  Score.prototype.handleUpdate = function () {
    entityUtil_1.entityUtil.setLabelText(this.label, String(this.value));
    this.animePlusScore();
    if (this.value < 0) {
      this.value = 0;
   ...
   * ゲーム終了時の処理まとめ
   */
  Score.prototype.handleFinishGame = function () {
